...
describe('Provider System', () => {
    describe('OrdiscanProvider', () => {
        ...
        describe('resolveInscription', () => {
            it('should return inscription data for a valid inscription ID', async () => {
                jest.spyOn(provider, 'fetchApi').mockResolvedValueOnce(mockResponses.ordiscan.inscription);
            ...
            it('should handle network errors', async () => {
                jest.spyOn(provider, 'fetchApi').mockRejectedValueOnce(new Error(`${ERROR_CODES.NETWORK_ERROR}: Request failed with status 500`));
        ...
        describe('getAllResources', () => {
            ...
            it('should handle empty response', async () => {
                jest.spyOn(provider, 'fetchApi').mockResolvedValueOnce({
                    data: []
                });
                const generator = provider.getAllResources({ batchSize: 2 });
                const result = await generator.next();
    ...
    describe('OrdNodeProvider', () => {
        ...
        describe('resolveInscription', () => {
            it('should return inscription data for a valid inscription ID', async () => {
                jest.spyOn(provider, 'fetchApi').mockResolvedValueOnce(mockResponses.ordNode.inscription);
                const result = await provider.resolveInscription('inscription1i0');
                expect(result).toEqual({
