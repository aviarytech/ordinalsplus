...
describe('Resource Creation Functions', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(psbtModule, 'createInscriptionPsbts').mockImplementation(async () => mockInscriptionResult as any);
  });
  ...
  describe('validateResourceCreationParams', () => {
    ...
    it('should throw an error if content type is missing', () => {
      ...
      expect(() => validateResourceCreationParams(params as any)).toThrow('Content type is required');
    });
    ...
    it('should throw an error if the network is unsupported', () => {
      ...
      expect(() => validateResourceCreationParams(params as any)).toThrow('Unsupported network');
    });
  });
  ...
  describe('createResourceTransaction', () => {
    it('should create a resource transaction successfully', async () => {
      ...
      const result = await createResourceTransaction(validParams as any, true);
    ...
    it('should handle errors from createInscriptionPsbts', async () => {
      ...
      await expect(createResourceTransaction(validParams as any, true)).rejects.toThrow('Failed to create resource transaction: Test error');
    });
  });
}); 